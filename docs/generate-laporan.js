...
const {
  ...
  PageBreak, TableOfContents, LevelFormat, PageNumber, Footer,
} = require('docx');
...
const quote = (text) => new Paragraph({
  spacing: { before: 120, after: 160, line: 300 }, indent: { left: 400 },
...
const li = (text, level = 0) => new Paragraph({
  numbering: { reference: 'peluru', level }, spacing: { after: 80, line: 290 },
  children: [new TextRun({ text, font: BODY, size: 22 })],
});
const num = (text) => new Paragraph({
  numbering: { reference: 'angka', level: 0 }, spacing: { after: 80, line: 290 },
  children: [new TextRun({ text, font: BODY, size: 22 })],
});
...
const cell = (text, { bold = false, fill = null, width } = {}) => new TableCell({
  width: { size: width, type: WidthType.DXA },
  shading: fill ? { type: ShadingType.CLEAR, fill } : undefined,
  margins: { top: 80, bottom: 80, left: 120, right: 120 },
  children: String(text).split(' ').map((t) => new Paragraph({
...
const gap = (n = 1) => Array.from({ length: n }, () => new Paragraph({ children: [new TextRun('')] }));

const NL = ' '; // pemisah baris di dalam sel tabel

/* ---------- muat isi bab dari berkas terpisah ---------- */
const bagian = require('./isi-laporan.js')({
  h1, h2, h3, p, rich, quote, li, num, code, caption, table, br, gap, NL,
});
...
const judul = [
  ...
  new Paragraph({
    ...
    children: [new TextRun({ text: 'LAPORAN TEKNIS', font: BODY, size: 28, bold: true, color: '595959' })],
  }),
  new Paragraph({
    alignment: AlignmentType.CENTER, spacing: { after: 100 },
    children: [new TextRun({ text: 'Pengembangan Authentication &', font: BODY, size: 48, bold: true, color: '1F3864' })],
  }),
  new Paragraph({
    alignment: AlignmentType.CENTER, spacing: { after: 300 },
    children: [new TextRun({ text: 'Authorization Service', font: BODY, size: 48, bold: true, color: '1F3864' })],
  }),
  new Paragraph({
    ...
    children: [new TextRun({
      ...
      font: BODY, size: 22, color: '44546A',
    })],
  }),
  ...gap(4),
  new Table({
    width: { size: 6200, type: WidthType.DXA }, columnWidths: [2200, 4000],
    ...
    rows: [
      ...
      children: [
        new TableCell({
          width: { size: 2200, type: WidthType.DXA }, margins: { top: 60, bottom: 60 },
          children: [new Paragraph({ children: [new TextRun({ text: k, font: BODY, size: 22, color: '595959' })] })],
        }),
        new TableCell({
          width: { size: 4000, type: WidthType.DXA }, margins: { top: 60, bottom: 60 },
          children: [new Paragraph({ children: [new TextRun({ text: v, font: BODY, size: 22, bold: true })] })],
        }),
      ],
    })),
  }),
    ...
    footers: {
      default: new Footer({
        children: [new Paragraph({
          alignment: AlignmentType.CENTER,
          children: [new TextRun({ children: [PageNumber.CURRENT], font: BODY, size: 18, color: '808080' })],
        })],
      }),
    },
